...
def marking(event):
    ...
    mark1 = tk.Scale(marktop,from_=0,  to=5,  resolution=0.5, orient=tk.HORIZONTAL , variable=var1 ,length = int(w_win*0.185),showvalue=0,tickinterval=1)
    mark1.place(x=0.01*w_win, y=0.058*h_win, anchor='w')

    tk.Label(marktop,text='2',bg='green').place(x=0, y=0.1*h_win, anchor='w')
    var2 = tk.DoubleVar()
    var2.set(2.5) 
    mark2 = tk.Scale(marktop,from_=0,  to=5,  resolution=0.5, orient=tk.HORIZONTAL , variable=var2,length = int(w_win*0.185),showvalue=0,tickinterval=1)
    mark2.place(x=0.01*w_win, y=0.10*h_win, anchor='w')

    tk.Label(marktop,text='3',bg='green').place(x=0, y=0.14*h_win, anchor='w')
    var3 = tk.DoubleVar()  
    var3.set(2.5) 
    mark3 = tk.Scale(marktop,from_=0,  to=5,  resolution=0.5, orient=tk.HORIZONTAL , variable=var3,length = int(w_win*0.185),showvalue=0,tickinterval=1)
    mark3.place(x=0.01*w_win, y=0.14*h_win, anchor='w')

    tk.Label(marktop,text='4',bg='green').place(x=0, y=0.18*h_win, anchor='w')
    var4 = tk.DoubleVar() 
    var4.set(2.5) 
    mark4 = tk.Scale(marktop,from_=0,  to=5,  resolution=0.5, orient=tk.HORIZONTAL , variable=var4,length = int(w_win*0.185),showvalue=0,tickinterval=1)
    mark4.place(x=0.01*w_win, y=0.18*h_win, anchor='w')

    b = tk.Button(marktop, text='上传打分',  command=upload)
